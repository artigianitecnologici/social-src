...
def run_server(port):
    ...
    tmux = TmuxSend('bringup', ['pantilt', 'joy4w'])
    ...
    while dorun:
        ...
        if data is not None:
            if len(data)==0:
                ...
                print(data)
                pfolder = os.getenv('MARRTINO_APPS_HOME')+"/camera/"
                if data=='@realsense_d345_start':
                    tmux.cmd(0,'cd %s' %pfolder)
                    tmux.cmd(0,'roslaunch realsense_d345.launch')
                
                elif data=='realsense_@d345_kill':
                    tmux.Cc(0)
                else:
